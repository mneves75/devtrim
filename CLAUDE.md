...
- Codex standalone releases are a third closed `agents` authority, `CODEX_RELEASES`, a `DeletionEntry` whose evidence is required. A release is offered only when the installer lock and `current` symlink prove an older direct-child layout-v1 package with canonical version numbers, and no process is executing any file inside it: one system-wide `lsof -d txt` taken under the lock, whose failure refuses release cleanup. Apply rechecks all of it and preserves current, newer, same-core, staging, running, and unknown layouts. Vendor files are opened no-follow, non-blocking and close-on-exec. A busy or unverifiable installer refuses only its release findings during apply.
