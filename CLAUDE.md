...
- `uninstall` matches by EXACT bundle identifier read from the bundle's own `Info.plist`, never by display name: on a real machine Amazon Kindle is `com.amazon.Lassen`, so a name match finds nothing and sweeps everything containing the word. It is report-only because `is_protected` refuses `/Applications` and all of `~/Library` outside its four-entry allowlist; making it delete would widen that boundary for every command, which needs its own decision.
- `optimize` tasks are fixed program plus fixed argv with no caller data, so there is no dynamic argument to validate. Root-requiring or hours-long tasks (`mdutil -E`, `periodic`, `purge`) stay out of the catalog and a test asserts it. Tasks are selectable with `--task` so one confirmation does not authorize both a resolver flush and a Launch Services rebuild; `optimize` is deliberately absent from `ops::all()` so `scan` never reports commands as reclaimable space.
