...
- Codex standalone releases are offered only when the installer lock and `current` symlink prove an older direct-child layout-v1 package; apply rechecks both and preserves current, newer, same-core, staging, and unknown layouts. A busy or unverifiable installer refuses only its release findings during apply.
