...
- `uninstall` matches by EXACT bundle identifier read from the bundle's own `Info.plist`, never by display name: on a real machine Amazon Kindle is `com.amazon.Lassen`, so a name match finds nothing and sweeps everything containing the word. It is a CONSERVATIVE REPORT, not an inventory — an app storing data under a product name (VS Code's `~/Library/Application Support/Code`) is invisible to identifier matching, and the output must keep saying so rather than claiming completeness. Group containers are not reported at all: their names come from an arbitrary `com.apple.security.application-groups` entitlement, so a filename-suffix rule both misses real ones and misattributes others. Report-only because `is_protected` refuses `/Applications` and all of `~/Library` outside its four-entry allowlist; making it delete would widen that boundary for every command, which needs its own decision.
- `optimize` tasks are fixed program plus fixed argv with no caller data, so there is no dynamic argument to validate. A task that cannot do what its name says stays out: root-requiring or hours-long ones (`mdutil -E`, `periodic`, `purge`) and DNS, because `dscacheutil -flushcache` does not clear the `mDNSResponder` resolver cache it would advertise. A test asserts none of them re-enter. `--apply` requires an explicit `--task` so one confirmation cannot authorize unrelated work, and `optimize` is deliberately absent from `ops::all()` so `scan` never reports commands as reclaimable space.
