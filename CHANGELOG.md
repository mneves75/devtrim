...
- A `~/.claude/projects/<project>` directory is offered only when every entry in it is session data: a `.jsonl` transcript, or a directory named for a canonical session id. Claude Code stores auto memory at `~/.claude/projects/<project>/memory/` and keys it by *repository root* while keying transcripts by *working directory*, so a project directory can hold memory and no live transcript, go stale, and carry away the one thing in the tree nothing can reconstruct. This is positive corroboration rather than a `memory` exception, so it also covers whatever an agent stores beside its transcripts next
