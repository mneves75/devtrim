        ...
        for finding in findings {
            ...
            let path = PathBuf::from(
                finding
                    .path
                    .as_deref()
                    .ok_or_else(|| anyhow::anyhow!("node_modules finding missing path"))?,
            );
            if !path.exists() {
                notes.push(format!("skipped vanished {}", path.display()));
                continue;
            }
            let owner = owning_repo(&path)
